...
// All and byName
const getAllDogsApi= async (id) => {
    try {
        const pedidoApi = await axios (`https://api.thedogapi.com/v1/breeds/${id}`);
...
const getById =async (id ) =>{
    console.log(id)
let res = await axios (`https://api.thedogapi.com/v1/breeds/${id}`)
...
//                     image: el.image.url,
//                     name: el.name,
//                     temperament: el.temperament? el.temperament : 'Perrito sin su  temperamento',
//                     weight: weigth1,
...
const createNewRace = async (req, res) => {
    try {
        ...
        const newRace = await Dog.create({
            ...
            height,
            weight,
            life_spam, // Aquí estaba life_spam, corregí el nombre
            image,
            createdInDb
        });
        ...
        res.status(400).json({ error: error.message }); // Asegúrate de enviar el error como objeto JSON
    }
};
